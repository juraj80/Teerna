...
import { colour, order } from '../../../shared';
...
export const Label = styled.label`
	...
	width: calc(220px);
	background-color: ${({ theme }) => theme.gmToggle[900]};
	border-radius: 100px;
	display: block;
	...
	& .bulb {
		...
		background-color: ${({ theme }) => theme.gmToggle[100]};
		border-radius: 50%;
		position: relative;
		top: calc(10px / 2);
		left: calc(10px / 1.75);
		display: block;
		transition: 0.7s;
		box-shadow: inset 0 0 1px 3px ${({ theme }) => theme.gmToggle[800]},
			inset 0 0 6px 8px ${({ theme }) => theme.gmToggle[200]},
		...
		& .bulb-centre {
			...
			background-color: ${({ theme }) => theme.gmToggle[300]};
			border-radius: 50%;
			top: 50%;
			left: 50%;
			transition: 0.7s;
			transform: translate(-50%, -50%);
			box-shadow: inset 0 0 0 4px ${({ theme }) => theme.gmToggle[400]};
			...
			&:after {
				...
				background-color: ${({ theme }) => theme.gmToggle[500]};
				border-radius: 50%;
				position: absolute;
				transition: 0.7s;
				top: 50%;
				left: 50%;
				transform: translate(-50%, -50%);
				box-shadow: 0 0 2px 4px ${({ theme }) => theme.gmToggle[600]};
			}
		}
		...
		& .filament {
			...
			&-2 {
				...
				&:before {
					...
					border-radius: 50%;
					border: 2px solid ${({ theme }) => theme.gmToggle[800]};
					position: absolute;
					transition: 0.7s;
					top: calc(-4px / 2);
					left: calc(-2px / 2);
		...
		css`
			.bulb {
				...
				box-shadow: inset 0 0 1px 3px #a56758,
					inset 0 0 6px 8px ${({ theme }) => theme.gmToggle[800]},
					0 20px 30px -10px rgba(0, 0, 0, 0.4),
				...
				& > .filament-1:after,
				& > .filament-2:after {
					border-color: ${({ theme }) => theme.gmToggle[100]};
				}
			}
		`};
`;
