...
const config = { ws: { domain: 'localhost', port: 8888, ssl: false } };
...
class WSConnection {
	constructor(onOpen = [], onMessage = [], onClose = []) {
		this.ws = new WebSocket(`${config.ws.ssl ? 'wss': 'ws'}://${config.ws.domain}:${config.ws.port}`);
	...
	onMessage(msg) {
		const { data } = msg;
		data && this.subscribers.message.forEach(f => f(msg));
	}
...
export function connection(onOpen, onMessage, onClose) {
	if (!conn) conn = new WSConnection(onOpen, onMessage, onClose);
	else {
