...
class Chat extends Component {
	...
	constructor(props) {
		super(props);
		this.ws = connection([], [e => this.onMessage(e)], []);
	}
	...
	addWhoIsTyping(message) {
		if (
			this.state.whoIsTyping.every(
				w => w.author && w.author.username !== message.author.username
			)
		) {
			this.setState({
				whoIsTyping: this.state.whoIsTyping.concat(message.author),
			});
			setTimeout(
				() =>
					this.setState({
	...
	getChatBoardContent() {
		const diceMessages = this.state.diceHistory
			? this.state.diceHistory.map(
					d =>
						new ChatMessage(
							`d${d.sides}: ${d.result}`,
							'GM',
	...
	render() {
		return (
			<div id='chat-box'>
				...
				<div
					id='who-is-typing'
					className={this.state.whoIsTyping.length === 0 ? '' : 'typing'}
				>
