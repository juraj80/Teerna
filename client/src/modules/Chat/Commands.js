...
const playerSpecific =
	'This command accepts player names as parameters to restrict the action to them.';
...
export function registerAccessControlCommands() {
	[
		...
		[
			'kick',
			'Log a player out of the game and block the player to log in again',
		],
...
export function playerCommands() {
	[['list', 'list of players CRUD']].forEach(d =>
		Command.register(d[0], 'player', d[1])
	);
}
...
export function sceneCommands() {
	[
		[
			'play',
			'Start playing the provided media to all players. You can specify which player should the media be played to by adding their names after the media',
		],
